# 记录所有的名片字典的列表变量
card_list = []


def show_menu():
    """显示菜单"""

    print("*" * 50)
    print("欢迎使用【名片管理系统】V 1.0")
    print("")
    print("1. 新增名片")
    print("2. 显示全部")
    print("3. 搜索名片")
    print("")
    print("0. 退出系统")
    print("*" * 50)


def new_card():
    """新增名片"""

    # 1. 提示用户已进入新增名片
    print("-" * 50)
    print("新增名片")

    # 2. 提示用户输入名片的详细信息
    name_str = input("请输入姓名：")
    phone_str = input("请输入电话：")
    qq_str = input("请输入QQ：")
    email_str = input("请输入邮箱：")

    # 3. 使用用户输入的信息建立一个名片字典
    card_dict = {"name": name_str,
                 "phone": phone_str,
                 "qq": qq_str,
                 "email": email_str}

    # 4. 将名片字典添加到列表中
    card_list.append(card_dict)

    # 5. 提示用户添加成功
    print("添加 %s 的名片成功！" % name_str)


def show_all():
    """显示所有名片"""

    # 1. 提示用户已进入显示所有名片
    print("-" * 50)
    print("显示所有名片")

    # 2. 判断是否存在名片记录，如果没有，提示用户并且返回
    if len(card_list) == 0:
        print("当前没有任何的名片记录，请使用新增功能添加名片！")

        # return 可以返回一个函数的执行结果
        # 下方的代码不会被执行
        # 如果 return 后面没有任何的内容，表示会返回到调用函数的位置
        # 并且不返回任何的结果
        return

    # 3. 打印表头
    # for name in ["姓名", "电话", "QQ", "邮箱"]:
    # print(name, end="\t\t")
    # print("%-10s" % name, end="\t")
    # print("")
    print("%-10s\t%-10s\t%-10s\t%-10s" % ("姓名", "电话", "QQ", "邮箱"))

    # 4. 打印分隔线
    print("=" * 50)

    # 5. 遍历名片列表依次输出字典信息
    for card_dict in card_list:
        print("%-10s\t%-10s\t%-10s\t%-10s" % (card_dict["name"],
                                              card_dict["phone"],
                                              card_dict["qq"],
                                              card_dict["email"]))


def search_card():
    """搜索名片"""

    # 1. 提示用户已进入搜索名片
    print("-" * 50)
    print("搜索名片")

    # 2. 提示用户输入要搜索的姓名
    find_name = input("请输入要搜索的姓名：")

    # 3. 遍历名片列表，查询要搜索的姓名，如果找到，针对找到的名片记录执行相应的操作
    for card_dict in card_list:

        if card_dict["name"] == find_name:

            # 3.1 显示查询到的名片
            # print("姓名\t\t电话\t\tQQ\t\t邮箱")
            print("%-10s\t%-10s\t%-10s\t%-10s" % ("姓名", "电话", "QQ", "邮箱"))
            print("=" * 50)
            print("%s\t\t%s\t\t%s\t\t%s" % (card_dict["name"],
                                            card_dict["phone"],
                                            card_dict["qq"],
                                            card_dict["email"]))

            action_str = input("[1] 修改名片 [2] 删除名片 [0] 返回上级菜单\n"
                               "请选择希望执行的操作: ")

            # 3.2 执行修改操作
            if action_str == "1":
                card_dict["name"] = input_card_info(card_dict["name"], "请输入姓名：")
                card_dict["phone"] = input_card_info(card_dict["phone"], "请输入电话：")
                card_dict["qq"] = input_card_info(card_dict["qq"], "请输入QQ：")
                card_dict["email"] = input_card_info(card_dict["email"], "请输入邮箱：")
                print("修改名片成功！")

            # 3.3 执行删除操作
            elif action_str == "2":
                card_list.remove(card_dict)
                print("删除名片成功！")

            break

    # 4. 如果没有找到，给用户相应的提示
    else:
            print("抱歉，没有找到相应名片 %s" % find_name)


def input_card_info(dict_value, tip_message):
    """输入名片信息

    :param dict_value: 字典中原有的值
    :param tip_message: 给用户的提示文字
    :return: 如果用户输入了内容，就返回内容，否则返回原有的值
    """

    # 1. 提示用户输入内容
    result_str = input(tip_message)

    # 2. 针对用户输入内容进行判断，如果用户输入了内容，则返回输入结果
    if len(result_str) > 0:
        return result_str

    # 3. 如果用户没有输入内容，则返回原有值
    else:
        return dict_value
